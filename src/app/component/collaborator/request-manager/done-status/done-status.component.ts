...
@Component({
  selector: 'app-done-status',
  templateUrl: './done-status.component.html',
  styleUrls: ['./done-status.component.css']
})
export class DoneStatusComponent implements OnInit {
  ...
  getData(){
    ...
    this.requestManagerService.getRequestStatus(this.collaboratorId, "done", searchForm).subscribe({
      next: res => {
        this.datas = res;
